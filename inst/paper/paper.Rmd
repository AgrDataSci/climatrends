...
Additionally, the function `GDD()` offers three modified equations (two) designed for cold environments and (one) for tropical environments. For cold environments, where $T_{min}$ may be lower than $T_{0}$, there are two variants of the previous equation to adjust either $T_{mean}$ (variant a) or $T_{min}$ (variant b). The variant a changes $T_{mean}$ to $T_{0}$ if $T_{mean} < T_{0}$ and is expressed as follows.
...
$$\parallel GDD_{ndays} \parallel \: = \; ggd_1 \;+ \; ...  \; +  \; gdd_n$$
